...
def sample_beta_features(probX, probE, node_mask, threshold=0.5, is_node_feat=False):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        :param probX: bs, n, dx_out        node features
        :param probE: bs, n, n, de_out     edge features
        :param proby: bs, dy_out           global features.
    ...
    # Sample X
    if is_node_feat:  # one-hot format
        X_t = torch.argmax(probX, dim=-1)
        X_t = F.one_hot(X_t, num_classes=probX.shape[-1]).float()
        X_t = X_t.reshape(bs, n, -1)  # (bs, n, -1)
    else:
        probX[torch.where(probX > threshold)] = 1.  # (bs * n, 1)
        probX[torch.where(probX <= threshold)] = 0.
        X_t = probX.reshape(bs, n, -1)  # (bs, n, 1)
    ...
    probE[inverse_edge_mask] = 0.
    probE[diag_mask.bool()] = 0.

    probE = probE.reshape(bs * n * n, -1)    # (bs * n * n, de_out)

    # Sample E
    probE[torch.where(probE > threshold)] = 1.
    probE[torch.where(probE <= threshold)] = 0.

    E_t = torch.triu(probE.reshape(bs, n, n), diagonal=1)  # (bs, n, n)
    E_t = (E_t + torch.transpose(E_t, 1, 2))
    E_t = F.one_hot(E_t.long(), num_classes=probE.shape[-1] + 1).float()  # (bs, n, n, 2)

    return PlaceHolder(X=X_t, E=E_t, y=torch.zeros(bs, 0).type_as(X_t))

def sample_beta_features_mol(probX, probE, node_mask, threshold=0.25, num_node_types=4, is_node_feat=False, ):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        :param probX: bs, n, dx_out        node features
        :param probE: bs, n, n, de_out     edge features
        :param proby: bs, dy_out           global features.
    ...
    # Sample E
    edge_exist = torch.max(probE, dim=-1)[0] > threshold
    E_t = torch.where(edge_exist, torch.argmax(probE, dim=-1) + 1, 0)

    E_t = torch.triu(E_t.reshape(bs, n, n), diagonal=1)  # (bs, n, n)
    E_t = (E_t + torch.transpose(E_t, 1, 2))
    E_t = F.one_hot(E_t, num_classes=probE.shape[-1] + 1).float()

    return PlaceHolder(X=X_t, E=E_t, y=torch.zeros(bs, 0).type_as(X_t))


def sample_beta_features_mol_1bit(X, E):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        :param X: bs, n, dx_out        node features
        :param E: bs, n, n, de_out     edge features
    '''

    E = E * 3
    E[E >= 2.5] = 3
    E[torch.bitwise_and(E >= 1.5, E < 2.5)] = 2
    E[torch.bitwise_and(E >= 0.5, E < 1.5)] = 1
    E[E < 0.5] = 0
    E = E.to(torch.int64)

    E = E - 1
    E[E == - 1] = 3
    E = F.one_hot(E.squeeze(-1), num_classes=4).permute(0, 3, 1, 2)

    X = torch.where(X > 0.5, 1, 0)
    X = torch.concat([X, 1 - torch.sum(X, dim=-1, keepdim=True)], dim=-1)
...
def sample_beta_feature_noise(limit_dist, node_mask):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    x_limit = limit_dist.X[None, None, :].expand(bs, n_max, -1)
    e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
...
def create_masks(node_mask):
    """
        expand node_mask to x_mask and e_mask:
        x_mask: (B, N, 1), mask for node feature matrix of the batch
        e_mask: (B, N, N), mask for the adj of the batch
    """
    B, N = node_mask.shape
    x_mask = node_mask.unsqueeze(-1)        # B * N * 1

    e_mask1 = node_mask.unsqueeze(1)        # B * 1 * N
    e_mask2 = node_mask.unsqueeze(2)        # B * N * 1
    diag_mask = torch.eye(N)
    diag_mask = ~diag_mask.type_as(node_mask).bool()
    diag_mask = diag_mask.unsqueeze(0).expand(B, -1, -1)        # B * N * N
    e_mask = (e_mask1 * e_mask2 * diag_mask).to(torch.float32)  # B * N * N

    triu_mask = torch.triu(torch.ones(N, N, dtype=torch.bool, device=node_mask.device), diagonal=1)
    triu_mask = triu_mask.unsqueeze(0).expand(B, -1, -1)

    return e_mask, x_mask, triu_mask
