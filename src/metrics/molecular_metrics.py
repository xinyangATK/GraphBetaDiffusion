...
class SamplingMolecularMetrics(nn.Module):
    def __init__(self, dataset_infos, train_smiles):
        ...
        self.generated_edge_dist = GeneratedEdgesDistribution(di.output_dims['E'] + 1)  # for Beta add "no edge"
    ...
    def forward(self, molecules: list, name, current_epoch, val_counter, local_rank, test=False):
        ...
        if local_rank == 0:
            ...
            write_mols = []
            for molecule in valid_unique_molecules:
                write_mols.append(molecule + '\n')
            # textfile.writelines(valid_unique_molecules)
            textfile.writelines(write_mols)
            textfile.close()
