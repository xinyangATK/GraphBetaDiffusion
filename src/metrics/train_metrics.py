...
class TrainLossBeta(nn.Module):
    """ Train with Cross entropy"""
    def __init__(self, lambda_train):
        super().__init__()
        self.node_loss = KLUBs()
        self.edge_loss = KLUBs()
        self.y_loss = CrossEntropyMetric()
        self.lambda_train = lambda_train


    def forward(self, masked_pred_X, masked_pred_E, pred_y, true_X, true_E, true_y, coefs, log: bool):
        """ Compute train metrics
        masked_pred_X : tensor -- (bs, n, dx)
        masked_pred_E : tensor -- (bs, n, n, de)
        pred_y : tensor -- (bs, )
        ...
        loss_X = self.node_loss(flat_pred_X, flat_true_X, coefs=coefs, z_mask=mask_X, n=num_nodes) if true_X.numel() > 0 else 0.0
        loss_E = self.edge_loss(flat_pred_E, flat_true_E, coefs=coefs, z_mask=mask_E, n=num_nodes**2) if true_E.numel() > 0 else 0.0
        loss_y = self.y_loss(pred_y, true_y) if true_y.numel() > 0 else 0.0

        if log:
            to_log = {"train_loss/batch_KLUB": (loss_X + loss_E + loss_y).detach(),
                      "train_loss/X_KLUB": self.node_loss.compute() if true_X.numel() > 0 else -1,
                      "train_loss/E_KLUB": self.edge_loss.compute() if true_E.numel() > 0 else -1,
                      "train_loss/y_CE": self.y_loss.compute() if true_y.numel() > 0 else -1}
            if wandb.run:
                wandb.log(to_log, commit=True)
        return loss_X + self.lambda_train[0] * loss_E + self.lambda_train[1] * loss_y
    ...
    def log_epoch_metrics(self):
        ...
        to_log = {"train_epoch/x_KLUB": epoch_node_loss,
                  "train_epoch/E_KLUB": epoch_edge_loss,
                  "train_epoch/y_CE": epoch_y_loss}
        if wandb.run:
            wandb.log(to_log, commit=False)
