...
def init_node_feats(X, E, feat_type='deg', max_feat_num=50):

    if feat_type == 'deg':
        deg = E.sum(dim=(-1, -2)).to(torch.long)
        feat = F.one_hot(deg, num_classes=max_feat_num).to(torch.float32)
    else:
        feat = X

    assert X.shape[0] == feat.shape[0]

    return feat

def compute_extra_feature(X, E, feat_type='deg', max_feat_num=50):
    ...
    if feat_type == 'deg':
        ...
        feat = X.new_zeros((*X.shape[:-1], 0))

    return feat

def init_node_feats_mol(X, E, feat_type='deg', max_feat_num=50):

    if feat_type == 'deg':
        deg = E.sum(dim=(-1, -2)).to(torch.long)
        feat = F.one_hot(deg, num_classes=max_feat_num).to(torch.float32)
        feat = torch.concat([X, feat], dim=-1)
    else:
        feat = X
    ...
    return feat
def compute_extra_feature_mol(X, E, feat_type='deg', max_feat_num=50):
